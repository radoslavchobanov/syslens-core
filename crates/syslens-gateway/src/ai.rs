...
pub(crate) fn storage_model_facts(facts: &RootStorageFacts) -> String {
    let mut directories = facts
        ...
        .collect::<Vec<_>>();
    // Reserve the compact model budget for eligible causal candidates first.
    // Raw delta ordering is not sufficient: a large pre-existing image can
    // otherwise evict the smaller, current-interval finding the model needs
    // to see in order to make a grounded interpretation.
    let selected_files = facts
        .file_facts
        .iter()
        .filter(|finding| finding.evidence_class.causal_eligible())
        .chain(
            facts
                .file_facts
                .iter()
...
fn storage_answer_has_unsupported_file_claim(answer: &str, facts: &RootStorageFacts) -> bool {
    ...
    fn phrase_is_negated(text: &str, phrase: &str) -> bool {
        ...
        while let Some(relative) = text[offset..].find(phrase) {
            ...
            let mut prefix_start = start.saturating_sub(56);
    ...
    fn token_is_negated(tokens: &[&str], index: usize) -> bool {
        ...
        fn is_negation_bridge(token: &str) -> bool {
            if token.is_ascii() && token.ends_with("ly") {
                return true;
            }
            matches!(
                token,
    ...
    fn has_following_token(tokens: &[&str], index: usize, candidates: &[&str]) -> bool {
        tokens
            .get(index + 1..(index + 10).min(tokens.len()))
            .is_some_and(|following| following.iter().any(|token| candidates.contains(token)))
    }

    fn has_negative_noun_modifier(tokens: &[&str], index: usize) -> bool {
        let following = match tokens.get(index + 1..(index + 10).min(tokens.len())) {
    ...
    fn has_token_level_causal_claim(clause: &str) -> bool {
        ...
        for (index, token) in tokens.iter().enumerate() {
            if !is_causal_verb(token)
                || token_is_negated(&tokens, index)
                || has_negative_noun_modifier(&tokens, index)
            {
                continue;
            }
    ...
    fn alias_token_character(character: char) -> bool {
        character.is_alphanumeric() || matches!(character, '_' | '-')
    }

    fn alias_occurrences(text: &str, alias: &str) -> Vec<(usize, usize)> {
        if alias.is_empty() {
            return Vec::new();
        }
        let mut occurrences = Vec::new();
        let mut search_from = 0;
        while let Some(relative) = text[search_from..].find(alias) {
            let alias_start = search_from + relative;
            let alias_end = alias_start + alias.len();
            let has_left_token = text[..alias_start]
                .chars()
                .next_back()
                .is_some_and(alias_token_character);
            let has_right_token = match text[alias_end..].chars().next() {
                Some('.') => text[alias_end + 1..]
                    .chars()
                    .next()
                    .is_some_and(alias_token_character),
                Some(character) => alias_token_character(character),
                None => false,
            };
            if !has_left_token && !has_right_token {
                occurrences.push((alias_start, alias_end));
            }
    ...
    fn segment_introduces_independent_subject(segment: &str) -> bool {
        fn is_measurement_predicate(token: &str) -> bool {
            token.starts_with("chang")
                ...
                || token.starts_with("grow") && token != "growth"
                || token.starts_with("increas") && token != "increase"
                || token.starts_with("observ")
                || token.starts_with("report")
                || token.starts_with("show")
                || matches!(
                    token,
                    "are"
                        | "appears"
    ...
    for (finding_index, finding) in facts.file_facts.iter().enumerate() {
        if finding.evidence_class.causal_eligible() {
            continue;
        }
        for (alias, _) in file_aliases
            .iter()
            .filter(|(_, owner)| *owner == finding_index)
        {
            for (alias_start, alias_end) in alias_occurrences(&answer, alias) {
                ...
                let mut boundaries = answer
                    ...
                    .collect::<Vec<_>>();
                for &(conjunction_start, conjunction_end) in &conjunctions {
                    ...
                    let following_end = boundaries
                        .iter()
                        .copied()
                        .filter(|boundary| *boundary > conjunction_end)
                        .min()
                        .or_else(|| {
                            conjunctions
                                .iter()
                    ...
                    let has_independent_subject_before = segment_introduces_independent_subject(
                        &answer[preceding_start..conjunction_start],
                    );
                    let has_independent_subject_after = segment_introduces_independent_subject(
                        &answer[conjunction_end..following_end],
                    );
                    if has_other_alias_before
                        || has_other_alias_after
                        || has_independent_subject_before
                        || has_independent_subject_after
                    {
                ...
                let has_unsupported_attribution = attribution_phrases
                    .iter()
                    .any(|phrase| clause.contains(phrase) && !phrase_is_negated(clause, phrase));
                let has_unsupported_timing = timing_phrases
                    .iter()
                    .any(|phrase| clause.contains(phrase) && !phrase_is_negated(clause, phrase));
                if has_unsupported_attribution
                    || has_unsupported_timing
                    || has_token_level_causal_claim(clause)
                    || has_token_level_timing_claim(clause)
                {
                    return true;
                }
            }
...
pub(crate) fn grounded_storage_fallback(answer: &str, facts: &RootStorageFacts) -> Option<String> {
    ...
    let change = if facts.used_bytes_change == 0 {
        format!("{direction} (delta 0 bytes, {:+.2} GiB)", gibibytes(0))
    } else {
        format!(
            "{direction} by {magnitude} bytes (delta {:+} bytes, {:+.2} GiB)",
            facts.used_bytes_change,
            gibibytes(facts.used_bytes_change)
        )
    };
...
mod tests {
    ...
    fn root_storage_facts_preserve_a_complete_zero_delta() {
        ...
        let fallback = grounded_storage_fallback("Storage increased by 1 byte.", &facts).unwrap();
        assert!(fallback.contains("root filesystem did not change"));
        assert!(!fallback.contains("decreased by 0 bytes"));
        assert!(!fallback.contains("increased by 0 bytes"));
    }
    ...
    fn storage_file_provenance_gate_reproduces_live_growth_case() {
        ...
        let model_verified = model_files
            .iter()
            .find(|file| {
                file["path"]
                    .as_str()
                    .unwrap()
                    .ends_with("verified-current-growth.img")
            })
            .unwrap();
        assert_eq!(model_verified["evidence_class"], "causal_candidate");
        assert_eq!(model_verified["causal_eligible"], true);
        assert!(
        ...
        assert!(
            grounded_storage_fallback(
                "minecraft-rpg.qcow2 did not cause 11 GB of storage growth.",
                &facts
            )
            .is_none()
        );
        ...
        assert!(
            grounded_storage_fallback("diagnosis.sqlite showed no growth today.", &facts).is_none()
        );
        assert!(
        ...
        assert!(
            grounded_storage_fallback("diagnosis.sqlite saw not an increase today.", &facts)
                .is_none()
        );
        assert!(
            grounded_storage_fallback(
                "minecraft-rpg.qcow2 was not materially responsible for storage growth.",
                &facts
            )
            .is_none()
        );
        ...
        assert!(
            grounded_storage_fallback(
                "minecraft-rpg.qcow2 wasn't active in the current window.",
                &facts
            )
            .is_none()
        );
        ...
        .is_none());
        assert!(grounded_storage_fallback(
            "minecraft-rpg.qcow2 is inventory-only; verified-current-growth.img is a likely contributor.",
            &facts
        )
        .is_none());
        assert!(grounded_storage_fallback(
        ...
        assert!(
            grounded_storage_fallback(
                "diagnosis.sqlite is inventory-only while the root filesystem showed growth today.",
                &facts
            )
            .is_none()
        );
        ...
        assert!(
            grounded_storage_fallback("minecraft-rpg.qcow2 was active and grew today.", &facts)
                .is_some()
        );
        assert!(grounded_storage_fallback(
    ...
    #[test]
    fn storage_claim_guard_handles_unicode_lookbehind_without_panicking() {
        let evidence = json!({
            "data": {
                "current": {"start_utc":"current-start","end_utc":"current-end"},
                "comparison": {"start_utc":"comparison-start","end_utc":"comparison-end"},
                "mounts": [{
                    "mount_id":"root-mount","mount_point":"/",
                    "current_used_bytes":1100i64,"comparison_used_bytes":1000i64,
                    "used_bytes_change":100i64
                }],
                "directories": [],
                "file_findings": [{
                    "mount_id":"root-mount","root":"/","path":"/var/данные.img",
                    "allocated_bytes_change":100i64,"apparent_bytes_change":100i64,
        ...
        assert!(
            grounded_storage_fallback("метаданные.img caused storage growth.", &facts).is_none()
        );
